...
Эта часть **не обязательна для зачёта**, но служит хорошей возможностью прокачаться и продемонстрировать инициативу. Можно выбрать **любой один** из вариантов (или придумать свой и кратко описать его в отчёте).

Примеры идей:

#### Вариант A. Новая CLI-команда `head` или `sample`

Добавить в `cli.py` команду, например:
...
```bash
uv run eda-cli sample data/my_data.csv --n 10
```

Требования:

- команда должна загружать CSV-файл;
- выводить первые `n` строк или случайную выборку `n` строк;
- иметь понятный help-текст и параметры (`--n`, при желании — `--sep`, `--encoding`).

#### Вариант B. JSON-сводка по датасету

Добавить к команде `report` опцию, например:

```bash
uv run eda-cli report data/my_data.csv --out-dir reports_my_data --json-summary
```

Которая, помимо существующих артефактов, сохраняет файл `summary.json` с компактной сводкой:

- размеры датасета (`n_rows`, `n_cols`);
- интегральный `quality_score`;
- список «проблемных» колонок (по пропускам, эвристикам качества и т.п.).

#### Вариант C. Дополнительная визуализация

Добавить в `viz.py` ещё один вид графика и интегрировать его в `report`:

- bar-chart по количеству объектов в каждой категории `country` (или другом категориальном признаке);
- boxplot для нескольких числовых признаков сразу;
- отдельный график по top-N категориям для выбранной колонки.

Требования:

- график должен сохраняться в файл PNG;
- ссылка на этот файл или его упоминание должны появиться в `report.md`.
