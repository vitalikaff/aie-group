...
def summarize_dataset(df: pd.DataFrame, example_values_per_column: int = 3) -> DatasetSummary:
    ...
    for name in df.columns:
        ...
        if is_numeric and non_null > 0:
            # явное приведение для безопасности
            min_v = float(s.min())
            max_v = float(s.max())
            mean_v = float(s.mean())
            std_v = float(s.std())
...
def compute_quality_flags(
    ...
    """
    Возвращает набор флагов качества и метрик.
    Включает:
      - too_few_rows, too_many_columns, too_many_missing
      - max_missing_share
      - has_constant_columns + список таких колонок
      - has_high_cardinality_categoricals + список таких колонок
      - missing_share (dict по колонкам)
      - quality_score: float в [0,1]
    """
    max_missing = float(missing_df["missing_share"].max()) if not missing_df.empty else 0.0

    # ========== НОВЫЕ эвристики ==========
    # 1) Константные колонки (включая все NaN, т.е. unique<=1)
    constant_columns = [c.name for c in summary.columns if c.unique <= 1]
    has_constant_columns = len(constant_columns) > 0

    # 2) Высокая кардинальность категориальных признаков (rel share threshold)
    ...
    # missing_share: словарь для удобства внешних пользователей (cli/api)
    missing_share = {}
    if not missing_df.empty:
        missing_share = {str(idx): float(v) for idx, v in missing_df["missing_share"].to_dict().items()}
    else:
        # если нет строк/столбцов — можно заполнить по колонкам summary
        missing_share = {c.name: float(c.missing_share) for c in summary.columns}

    # ========== Простой quality_score ==========
    ...
    # Небольшой штраф за количество колонок с пропусками > 20%
    n_bad_missing = sum(1 for v in missing_share.values() if v > 0.2)
    score -= min(0.2, 0.02 * n_bad_missing * 10)  # мягкий штраф
